...
#----------------------------------------------------------------------
def getWindData(InstrumentCode, initDay):
    """取指数数据"""
    w.start()
    w.start(waitTime=60)
    ...
    IndexData = w.wsi(InstrumentCode, "open,high,low,close,volume,oi", datetime.today() - timedelta(initDay), datetime.today()+ timedelta(1), "")
...
def runChildProcess():
    ...
    # db_bar = getWindData("hc1905.SHF",100)
    # loadWindCsv(db_bar, 'VnTrader_1Min_Db', 'rb1901')
